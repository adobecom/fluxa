...
6. **DO NOT use `colorLookup` / 3D-LUT preset layers** (e.g. "Color Lookup > Crisp Winter"). Those reference local `.3DL`/`.cube` preset files that DO NOT exist in the API and are silently ignored. For "Color Lookup" or LUT-style color grading, use `colorBalance` (`adjustments/color_balance.md`) and/or `curves` instead.

## ✂️ SUBJECT CUTOUT POLICY (CRITICAL — DO NOT SKIP)

Most composite/portrait effects (double exposure, dramatic composite, background
swap, "put the person on a texture", etc.) DEPEND on isolating the subject FIRST.
The API cannot use the Quick Selection tool, the "Select and Mask" workspace, or
"Refine Edge" — but it HAS an AI Select Subject that replaces all of them.

**Whenever the transcript mentions ANY of these** — "select the person / model /
subject", "quick selection", "select and mask", "refine edge", "cut out",
"remove background", "make a selection of the [subject]", "output to new layer
with layer mask", "load the selection of the [subject]" — you MUST emit the
cutout as REAL actions. Do NOT skip it and do NOT just place another image.

**Mandatory cutout sequence (read `selection/select_subject.md` +
`layer_management/create_mask.md`):**
```json
[
...
```
- Ignore "Select and Mask" / "Refine Edge" / "Refine Hair" as separate steps —
  `autoCutout` already produces a refined AI selection. Just do `autoCutout`
  then the `revealSelection` mask.
- "Load the selection of the subject" (Ctrl+Click a layer thumbnail) later in a
  tutorial → re-run `autoCutout` to regenerate that selection before using it.
- The very FIRST real editing step of a cutout/composite tutorial should almost
  always be `autoCutout`. If your output for such a tutorial has no `autoCutout`,
  you have made a mistake — add it.

## 🖌️ BRUSH / MANUAL MASKING SUBSTITUTION POLICY (CRITICAL)

The Photoshop API **cannot perform freehand brush strokes**. Tutorials constantly say things
like "select the brush, black foreground, and paint over the mask to hide/reveal this area."
If you emit a `hideAll`/`revealAll` mask and then a brush tool selection, the mask is NEVER
painted — the layer ends up fully hidden (hideAll) or unchanged (revealAll), and the effect
DOES NOT render. This is the #1 cause of broken results.

**NEVER do this:**
- ❌ Do NOT emit `{"_obj":"select","_target":[{"_ref":"paintbrushTool"}]}` (or any brush tool
  selection) — it does nothing without strokes.
- ❌ Do NOT create a `hideAll` mask expecting it to be painted white later.
- ❌ Do NOT create a `revealAll` mask expecting it to be painted black later.

**INSTEAD, substitute the brush's INTENT with these deterministic, automatable actions:**

1. **"Paint black to delete/hide the DARK background of an overlay so it blends"**
   (the classic double-exposure move) → set that layer's **blend mode to `screen` or `lighten`**
   via `set_layer_properties`. Screen/Lighten automatically drop the dark pixels — no mask needed.

2. **"Reveal only the subject / hide the overlay everywhere except the subject"** →
   run `autoCutout` (Select Subject), then create a mask with **`revealSelection`** on that layer
   (read `layer_management/create_mask.md`). Never use `hideAll` + brush for this.

3. **"Softly fade/blend across a region"** → prefer reducing layer **opacity** and using a
   blend mode. (Do NOT attempt gradient-on-mask unless a doc gives you exact JSON — an invalid
   descriptor breaks the whole render.)

4. **Always keep the layer VISIBLE and blended.** Reduce `opacity` as the tutorial specifies,
   set the stated blend mode, and rely on `revealSelection` masks — so every layer contributes
   to the final image.
...
2. **SECOND - IDENTIFY OPERATIONS**: Based on the transcript, identify which operations are needed and find their exact file paths from the index:
   - "duplicate layer" → `layer_management/duplicate_layer.md`
   - "select subject" / "select the person/model" / "quick selection" / "select and mask" / "refine edge" / "cut out" → `selection/select_subject.md` (use `autoCutout`; then `create_mask.md` with revealSelection) — NEVER skip the cutout
   - "add mask" → `layer_management/create_mask.md`
   - "color balance" → `adjustments/color_balance.md`
   - "render clouds" → `filters/render_clouds.md`
   - "add text" → `text_layer.md`
