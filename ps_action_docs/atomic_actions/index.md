...
> ⚠️ **If the tutorial mentions a BRUSH / painting on a mask** (e.g. "select the brush and paint black/white on the mask to hide/reveal"), READ **`brush_masking_substitution.md`** FIRST. The API cannot brush — you must substitute blend modes / selection masks, or the effect will not render.
...
| `adjustments/color_balance.md` | Color-grade shadows/midtones/highlights (cinematic mood) |
...
| `filters/render_clouds.md` | Render a fog/cloud pattern (Filter > Render > Clouds) — not Difference Clouds |
...
| `layer_management/layer_via_copy.md` | Copy the current SELECTION to a new layer (Ctrl+J after Select Subject) — NOT the same as duplicate |
...
| `layer_management/solid_color_fill_layer.md` | Create a Solid Color fill layer (Layer > New Fill Layer > Solid Color) |
...
|-----------|--------------|
| Brush painting on a mask (hide/reveal by brushing) | `brush_masking_substitution.md` ⚠️ read this — API can't brush |
| Select person/model, quick selection, select & mask, refine edge, cut out | `selection/select_subject.md` (use `autoCutout` → `create_mask.md`) — NEVER skip |
| Color grade shadows/midtones/highlights | `adjustments/color_balance.md` |
| Render clouds / fog | `filters/render_clouds.md` |
| Duplicate layer (whole layer, no selection) | `layer_management/duplicate_layer.md` |
| Ctrl+J / Layer Via Copy (copy the SELECTION to a new layer) | `layer_management/layer_via_copy.md` |
| Solid Color fill layer | `layer_management/solid_color_fill_layer.md` |
