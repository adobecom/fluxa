...
def process_with_actionjson(input_images, action_json_file, output_path=None):
    ...
    # Prepare output paths in R2 (PSD + PNG preview for browser display)
    output_filename = os.path.basename(input_images[0])
    base_name = os.path.splitext(output_filename)[0]
    output_r2_key_psd = f"output_{timestamp}_{unique_id}_{base_name}.psd"
    output_r2_key_png = f"preview_{timestamp}_{unique_id}_{base_name}.png"
    output_url_psd = generate_r2_presigned_url(r2_client, output_r2_key_psd, operation='put_object')
    output_url_png = generate_r2_presigned_url(r2_client, output_r2_key_png, operation='put_object')
    
    print(f"\n[OUTPUT] PSD will be: {output_r2_key_psd}")
    print(f"[OUTPUT] PNG preview will be: {output_r2_key_png}")
    ...
    # We request both PSD (full output) and PNG (browser-viewable preview)
    data = {
        "inputs": [{
            "storage": "external",
            "href": input_urls[0]
    ...
    # Poll for completion (check all outputs)
    print("\n[ADOBE] Polling for job completion...")
    max_attempts = 60
    attempt = 0
    all_succeeded = False
    
    while not all_succeeded and attempt < max_attempts:
        time.sleep(2)
        attempt += 1
        ...
        # Check status of all outputs
        statuses = [o.get('status', 'pending') for o in outputs]
        print(f"  Statuses: {statuses} (attempt {attempt}/{max_attempts})")
    ...
    if not all_succeeded:
        print(f"\nError: Job timed out or failed")
        print(json.dumps(job_result, indent=2))
        return None
    
    print("\n[SUCCESS] Job completed successfully!")
    
    # Download results from R2 (both PSD and PNG preview)
    ...
    print(f"\n[DOWNLOAD] Downloading PSD to: {output_path}")
    print(f"[DOWNLOAD] Downloading PNG preview to: {preview_path}")
    os.makedirs(output_dir, exist_ok=True)
    
    download_from_r2(r2_client, output_r2_key_psd, output_path)
    download_from_r2(r2_client, output_r2_key_png, preview_path)
    
    # Cleanup R2 files
    print("\n[CLEANUP] Cleaning up temporary files...")
    for key in r2_keys + [output_r2_key_psd, output_r2_key_png]:
        try:
            r2_client.delete_object(Bucket=R2_BUCKET_NAME, Key=key)
        except Exception as e:
            print(f"  Warning: Could not delete {key}: {e}")
    
    print(f"\n[COMPLETE] PSD saved to: {output_path}")
    print(f"[COMPLETE] PNG preview saved to: {preview_path}")
    
    # Return dict with both paths for server compatibility
    return {
...
if __name__ == '__main__':
    ...
    if result:
        if isinstance(result, dict):
            print(f"\n✓ Success!")
            print(f"  PSD: {result.get('output_path')}")
